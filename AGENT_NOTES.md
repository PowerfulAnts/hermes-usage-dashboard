...
Command Code sessions, bridge ledger) are deliberately NOT counted.
...
rate**, plus a total hit rate across all providers. IMPORTANT USER CORRECTION:
Hermes-only token scope does NOT mean removing usage bars, provider design,
usage limits, credits or funds. Those account-status views must remain.

Account status is independent from token scope: Codex rollout metadata may
be tail-read for subscription windows (never token totals); Command Code and
OpenRouter APIs supply limits/credits; Hermes supplies Nous Portal funds.
...
Every used provider has prompt volume, so zero cache reads correctly produce
`hit_rate_pct: 0.0` and an empty bar. `null` is reserved for no prompt volume.
...
- `dashboard/sources.py` — pure-stdlib aggregator. `collect(days)` is the
  Hermes-only token path; `collect_limits()` collects independently trackable
  provider account status with a 5-minute network cache.
...
- The sqlite aggregate is milliseconds-cheap (~200 rows live); token totals
  have NO TTL cache or background scan threads. Network-backed account status
  has a 5-minute TTL so the 20s UI refresh does not repeatedly hit APIs.
